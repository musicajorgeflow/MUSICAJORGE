...
const REQUEST_CONFIG = {
  // Optional: set this to a Formspree/Google Apps Script endpoint if you want
  // requests to be stored automatically online. Without it, requests are
  // saved locally and can also be exported as a TXT file from this browser.
  endpoint: ""
};
...
function render() {
  ...
  el.songs.innerHTML = filtered.map((s, i) => {
    ...
    return `<div class="song ${active ? "active" : ""}" data-i="${realIndex}">
      ...
      <div class="song-main">
        ${s.cover ? `<img class="thumb" src="${esc(s.cover)}" alt="" onerror="this.style.display='none'">` : `<div class="thumb"></div>`}
        <div>
...
function getLocalRequests() {
  try { return JSON.parse(localStorage.getItem("jorgeflow_requests") || "[]"); } catch { return []; }
}
function saveLocalRequest(text) {
  const arr = getLocalRequests();
  arr.push({ request: text, date: new Date().toISOString() });
  localStorage.setItem("jorgeflow_requests", JSON.stringify(arr));
}
async function submitRequest(text) {
  ...
  saveLocalRequest(text);
  if (REQUEST_CONFIG.endpoint) {
    try {
      await fetch(REQUEST_CONFIG.endpoint, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ request: text, date: new Date().toISOString() })
      });
      el.requestStatus.textContent = "Petición enviada.";
    } catch {
      el.requestStatus.textContent = "Guardada localmente. El envío online no respondió.";
    }
  } else {
    el.requestStatus.textContent = "Petición guardada en este navegador.";
  }
  el.manualRequest.value = "";
  show("Petición guardada");
}

// Export helper: from the same browser, save the current request inbox as TXT.
function exportRequestsTxt() {
  const rows = getLocalRequests();
  const text = rows.length ? rows.map(x => `[${x.date}] ${x.request}`).join("\n") : "No hay peticiones.";
  const blob = new Blob([text], { type: "text/plain;charset=utf-8" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob); a.download = "pedidas.txt"; a.click();
