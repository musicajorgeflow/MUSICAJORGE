...
// Icons written as \uXXXX escapes on purpose (not literal characters). Some
// hosting/serving setups (GitHub Pages, some browsers reading a .js file
// without an explicit charset) can misread raw multi-byte UTF-8 characters
// sitting inside a .js file, which is what caused the play button to briefly
// show garbled text like "â-¶" instead of "▶". Escapes are pure ASCII in the
// file itself, so there is nothing left to misinterpret.
const ICON = {
  play: "\u25B6",   // ▶
  pause: "\u2161",  // Ⅱ
  note: "\u266A",   // ♪
...
function render() {
  ...
  el.songs.innerHTML = filtered.map((s, i) => {
    ...
    return `<div class="song ${active ? "active" : ""}" data-i="${realIndex}">
      <div class="num">${active && !audio.paused ? ICON.play : i + 1}</div>
      <div class="song-main">
...
function load(i, auto = false) {
  ...
  if (s.cover) {
    // If the cover file 404s (missing/renamed on disk) fall back to the
    // note icon instead of showing a broken-image glyph.
    el.cover.onerror = () => {
...
async function play() {
  ...
  catch { show(`Pulsa ${ICON.play} para iniciar la reproducción.`); }
}
...
// If a track's audio file 404s or otherwise fails to load, don't just sit
// there silently: tell the user and automatically move on to the next song
// so a single broken file doesn't look like "the whole player is broken".
let lastErrorIndex = -1;
audio.onerror = () => {
  if (state.current < 0 || state.current === lastErrorIndex) return;
  lastErrorIndex = state.current;
  const s = base()[state.current];
  show(`No se pudo cargar "${s ? s.title : "esta canción"}", pasando a la siguiente...`);
