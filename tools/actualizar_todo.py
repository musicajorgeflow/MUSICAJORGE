...
def generate_library(folder, r2_folder, cfg):
    ...
    for path in sorted(music.rglob("*"), key=lambda x: x.name.lower()):
        ...
        try:
            ...
            title = first(f_easy, "title") or path.stem
            artist = first(f_easy, "artist") or "Artista desconocido"
            album = first(f_easy, "album")
        ...
        audio_field = (base_url + quote(path.name)) if remote else rel(path)
...
def upload_audio(cfg):
    ...
    state = load_state()
    subido = 0
    for folder, key in (("playlist_jorge", "jorge"), ("playlist_djgeeorge", "djgeeorge")):
